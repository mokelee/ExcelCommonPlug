...
| ID | 功能 | 描述 |
|----|------|------|
| FR-8.1 | 启动时自动初始化日志 | 每次 Excel 加载插件时初始化日志系统，生成独立日志文件（按时间戳命名） |
| FR-8.2 | 日志存储在安装路径 | 日志文件存储在 `{安装路径}\logs\` 目录下 |
| FR-8.3 | 保留最近7天日志 | 每次启动时自动清理超过7天的日志文件，7天内的全部保留 |
| FR-8.4 | 未处理异常捕获 | 注册 AppDomain.UnhandledException 和 Application.ThreadException，捕获聚光灯等功能导致的异常崩溃并写入日志 |
| FR-8.5 | Release构建启用日志 | 日志系统在所有构建模式下启用，不再仅限 DEBUG |
...
| NFR-6 | 可观测性 | 异常崩溃时有完整日志可供排查，日志不影响正常性能 |
| NFR-7 | 国际化 | UI 使用中文，代码注释中文 |
